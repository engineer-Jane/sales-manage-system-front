...
const TableList: React.FC = () => {
  ...
  const onJump = (id?: string) => {
    let params = '';
    if (id) params = `?id=${id}`;
    history.push(`/stock/retrieval/add${params}`)
  }
  ...
  const getPage = async (params: any) => {
    ...
    const request = {
      ...
      purchaseStockOrderType: 'PURCHASE_RETRIEVAL',
      ...parameter
    };
    setParam(request);
  ...
  const columns: ProColumns<API.TableItem>[] = [
    ...
    },
    {
      ...
      render: (t, r) => {
        return (
          <Space>
            <Access accessible={access.auth('stock.retrieval.edit')}>
              <a onClick={() => onJump(r?.purchaseStockId)}>编辑</a>
            </Access>
            <Access accessible={access.auth('stock.retrieval.delete')}>
  ...
  return (
    <PageContainer>
      <ProTable<API.TableItem, API.Params>
        ...
        headerTitle={
          <Access accessible={access.auth('stock.retrieval.add')}>
        ...
        toolBarRender={() => [
          <Access key="export" accessible={access.auth('stock.retrieval.export')}>
